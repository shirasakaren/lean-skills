...
> 📄 Source: https://lean-lang.org/faq/

## Frequently Asked Questions

This page provides answers to frequent questions and addresses common misconceptions and myths around Lean.

### Is Lean only useful for formalizing mathematics? {#faq-not-just-math}

Lean’s mathematical library, [Mathlib](https://github.com/leanprover-community/mathlib4), is impressive, but Lean itself is useful not just for verifying mathematics, but equally suitable for verifying software, hardware, protocols, and more. Lean is also a powerful tool for software development.

### Is Lean only a theorem prover? {#faq-not-just-prover}
...
### Is Lean just like Rocq? {#faq-rocq}

On a foundational level, Lean and [Rocq](https://rocq-prover.org/) (the theorem prover formerly known as Coq) have much in common: both are based on dependent type theory (Calculus of Inductive Constructions), with some technical differences:

- Lean uses definitional proof irrelevance.
...
### Is Lean’s logic classical? {#faq-classical}
...
### Is Lean just like Isabelle? {#faq-isabelle}

On a foundational level, Lean and [Isabelle/HOL](https://isabelle.in.tum.de/) are quite different: Lean is based on dependent type theory and constructs explicit proofs passed to a trusted small kernel, while Isabelle/HOL is based on higher-order logic (HOL) and uses the LCF approach.

### Are dependent types natural for formalizing mathematics? {#faq-dependent-types}
...
The success of Lean’s mathematical library and its adoption by mathematicians from the level of undergraduate teaching to Fields medalists shows that dependent type theory is not an obstacle but a boon for formalizing mathematics.

### Are proof objects a bottleneck? {#faq-proof-objects}

Lean works with proof objects: The user-written proof scripts and tactics are executed to produce an explicit term in the foundational logic that represents that proof, which is then passed to a small trusted component (the kernel) for verification. This way, the soundness of Lean as a theorem prover does not depend on the correctness of the elaboration steps and tactics.

The performance impact of proof objects in practice is less than often assumed, due to a number of engineering choices in Lean: The Lean kernel supports sharing of subterms in proof objects and lazily evaluates them. Proof terms of user theorems are usually not loaded when importing modules. With Lean’s module system they need not even be distributed with libraries.

The use of proof objects brings important advantages:

- Proof objects can be exported and then checked independently from the main Lean process, by a separate process, or even on separate machines or at a later time. This is more important than ever with the advent of large AI-generated proofs. For example, [Nanoda](https://github.com/ammkrn/nanoda_lib) is type/proof checker for Lean implemented in Rust.
- Additional optimizations such as parallel tactic execution can be implemented without having to extend the kernel.
...
### How do I validate a Lean proof? {#faq-validation}

The answer to that question depends on your threat model: Did you write the proof yourself, or someone you trust, or someone who is trying to trick you. More detailed guidance on what a Lean proof can be found in the [section “Validating a Lean Proof” of the Lean Reference Manual](https://lean-lang.org/doc/reference/latest/find/?domain=Verso.Genre.Manual.section&name=validating-proofs).

### What is the Trusted Code Base of Lean? {#faq-tcb}

One of Lean's core design approaches is that **you do not need to trust Lean's implementation** to trust the proofs it produces.
Proofs can be verified with independent checkers as described in more detail in the [section “Validating a Lean Proof” of the Lean Reference Manual](https://lean-lang.org/doc/reference/latest/find/?domain=Verso.Genre.Manual.section&name=validating-proofs).
Thus it is sufficient to trust in the soundness of the used checker after validating its output.
This base of trust, i.e. the source code of the used checker(s), is often called the *Trusted Code Base (TCB)*.
Multiple checkers can be used to further increase confidence.

Relying on independent checkers as the TCB covers many core applications of Lean where the proof is the main product:

- Mathematical theorems.
- Verification of existing algorithms, protocols, or hardware.
- Verification of software written in other languages and transformed into Lean statements via tools such as [Aeneas](https://github.com/AeneasVerif/aeneas).
  The tool in question may have additional assumptions about how the Lean statements relate to the original software, which become part of the TCB.

An extended use case of Lean that comes with an extended TCB is the combination of Lean as a theorem prover and as a programming language:

- Running verified Lean code as native binaries.
- Using the `native_decide` tactic (which is tracked as an extra axiom for visibility).

In these cases, the TCB is extended with the Lean compiler (written in Lean), the Lean runtime (written in C++), and the code generation backend (LLVM by default), as well as any other code external to Lean that is linked in.
Standard hardening techniques can be used to increase confidence in these components, especially when combining verified and unverified code; see also the `LEAN_MI_SECURE` CMake configuration option for allocator hardening of Lean 4.31.0+.

### Is it safe to build Lean code and libraries? {#faq-supply-chain}

Lean is a general purpose programming language with rich compile-time features.
Therefore building a Lean project, opening a Lean file or running commands like `lake update` can execute arbitrary code from the project or any of its dependencies.
This “supply chain attack” vector exists in essentially all programming language ecosystems, [including LaTeX](https://hovav.net/ucsd/dist/tex-login.pdf).
If you need to build code that may be actively malicious, do so in a properly isolated environment (e.g. a virtual machine).

### Does dependent type theory hinder SMT integration or other strong automation? {#faq-smt}
...
### Why is 1/0 = 0? {#faq-division-by-zero}
...
> 📄 Source: https://lean-lang.org/documentation/semantic-tokens/
...
However this may be insufficient if your color theme does not distinguish enough syntax categories or distinguishes them very subtly. For instance the default `Light+` theme uses color `#001080` for variables. This is awfully close to `#000000` that is used as the default text color. This makes it very easy to miss an accidental use of [auto bound implicit arguments](https://lean-lang.org/doc/reference/latest/find/?domain=Verso.Genre.Manual.section&name=automatic-implicit-parameters). For instance in
...
If you rather write `(n : Nat)` then `n` keeps its variable color but `Nat` gets the default text color.
...
"editor.semanticTokenColorCustomizations": {
  "[Default Light+]": {
    "rules": {"function": "#ff0000", "property": "#00ff00", "variable": "#ff00ff"}
  }
},
```
...
> 📄 Source: https://lean-lang.org/documentation/latex-syntax-highlighting/
...
### Example with `listings` {#latexhl-example-with--listings}
...
### Example with `minted` {#latexhl-example-with--minted}
...
- If you don't have FreeMono installed, then you can try any other font by writing its name directly. `xelatex` uses the operating system's fonts.
...
> 📄 Source: https://github.com/leanprover/vscode-lean4/blob/master/vscode-lean4/manual/manual.md
...
1. [Setting up Lean 4](#vsc-setting-up-lean-4)
1. [Configuring settings](#vsc-configuring-settings)
1. [Using commands](#vsc-using-commands)
    - [Command menu](#vsc-command-menu)
    - [Command palette](#vsc-command-palette)
    - [Chords](#vsc-chords)
    - [Configuring keyboard shortcuts](#vsc-configuring-keyboard-shortcuts)
1. [Interacting with Lean files](#vsc-interacting-with-lean-files)
    - [File processing](#vsc-file-processing)
    - [Errors, warnings and information](#vsc-errors-warnings-and-information)
    - [File restarting](#vsc-file-restarting)
    - [Unicode input](#vsc-unicode-input)
    - [InfoView](#vsc-infoview)
    - [Hovers](#vsc-hovers)
    - [Auto-completion](#vsc-auto-completion)
    - [Code actions](#vsc-code-actions)
    = [Inlay hints](#vsc-inlay-hints)
    - [Occurrence highlighting](#vsc-occurrence-highlighting)
    - [Semantic highlighting](#vsc-semantic-highlighting)
    - [Go to symbol](#vsc-go-to-symbol)
    - [Document outline](#vsc-document-outline)
    - [Breadcrumbs](#vsc-breadcrumbs-bar)
    - [Collapsible code blocks](#vsc-collapsible-code-blocks)
    - [Sticky scroll](#vsc-sticky-scroll)
1. [Navigating Lean projects](#vsc-navigating-lean-projects)
    - [Go to definition, declaration and type definition](#vsc-go-to-definition-declaration-and-type-definition)
    - [Call hierarchy](#vsc-call-hierarchy)
    - [Find references](#vsc-find-references)
    - [Workspace symbol search](#vsc-workspace-symbol-search)
    - [Project text search](#vsc-project-text-search)
    - [Module hierarchy](#vsc-module-hierarchy)
    - [Copy module name](#vsc-copy-module-name)
    - [Go to file](#vsc-go-to-file)
    - [Explorer](#vsc-explorer)
1. [Managing Lean projects](#vsc-managing-lean-projects)
    - [Creating projects](#vsc-creating-projects)
    - [Opening projects](#vsc-opening-projects)
    - [Nested projects](#vsc-nested-projects)
    - [Project actions](#vsc-project-actions)
    - [Lakefile editing](#vsc-lakefile-editing)
    - [Terminal](#vsc-terminal)
1. [Managing Lean versions](#vsc-managing-lean-versions)
    - [Elan](#vsc-elan)
    - [Selecting default Lean versions](#vsc-selecting-default-lean-versions)
    - [Updating Lean release channels](#vsc-updating-lean-release-channels)
    - [Uninstalling Lean versions](#vsc-uninstalling-lean-versions)
1. [Finding definitions and theorems](#vsc-finding-definitions-and-theorems)
    - [LoogleView](#vsc-loogleview)
1. [Troubleshooting issues](#vsc-troubleshooting-issues)
    - [Setup diagnostics](#vsc-setup-diagnostics)
    - [Collecting setup information](#vsc-collecting-setup-information)
    - [Output view](#vsc-output-view)
    - [Restarting Lean](#vsc-restarting-lean)
...
When no Lean 4 document has been selected yet, only specific commands that do not need an associated Lean 4 document can be used. This includes [creating Lean projects](#vsc-creating-projects), [opening Lean projects](#vsc-opening-projects), [displaying troubleshooting information](#vsc-troubleshooting-issues), [managing Lean versions](#vsc-managing-lean-versions) and various documentation commands, like the command to open this manual.

When a Lean 4 document has been selected once, additional commands that can only operate in the context of a Lean 4 document become available. This includes [restarting files](#vsc-file-restarting), [restarting Lean](#vsc-restarting-lean), [toggling the InfoView](#vsc-infoview) and [executing project actions](#vsc-project-actions) in the Lean project associated with the given Lean 4 document. When executing one of these commands, the command will operate in the context of the last Lean 4 document that has been focused in VS Code.
...
For parts of the file that are currently still being processed, Lean cannot provide any interactive features that require file processing information yet. For example, in unprocessed parts of the file, [errors, warnings and information](#vsc-errors-warnings-and-information) are not displayed, the [InfoView](#vsc-infoview) will not show the current goal state and [hovers](#vsc-hovers) will not display a popup until file processing has progressed to the respective part of the file.
...
Hovering over code that is underlined with a squiggly line will display the contents of the error, warning or information in a [hover popup panel](#vsc-hovers). 

Positioning the text cursor on the source code line containing the squiggly line will also make an interactive version of the content of the diagnostic show up in the 'Messages' section of the [InfoView](#vsc-infoview). The 'All Messages' section displays all interactive diagnostic contents of the whole file.
...
Instead, the state of all dependencies of `Module.lean` needs to be updated manually using the ['Server: Restart File'](command:lean4.restartFile) command. This will trigger a build for all changed dependencies. The command can be called using `Ctrl+Shift+X` (`Cmd+Shift+X`), using the 'Restart File' button in the InfoView, using the [command menu](#vsc-command-menu), using the [command palette](#vsc-command-palette) or by right clicking in the editor of a Lean file and selecting the 'Server: Restart File' entry in the context menu.

VS Code will display [error- or information-level diagnostics](#vsc-errors-warnings-and-information) whenever a dependency of a file is edited and saved. When initially opening a file and a dependency would need to be rebuilt to be up-to-date, it issues an error and refuses to [process](#vsc-file-processing) the rest of the file. When a dependency changes while the file is already open, it issues an information-level diagnostic, but will continue processing the file with the previous state of the dependency.
...
The full list of supported abbreviation identifiers and Unicode symbols can be viewed using the ['Docs: Show Unicode Input Abbreviations'](command:lean4.docs.showAbbreviations) command that can be found in the 'Documentation…' submenu of the command menu. When encountering a Unicode symbol in Lean code, [hovering](#vsc-hovers) over the symbol will also provide all available abbreviation identifiers to input the symbol.
...
To use Unicode symbols outside of text editors - for example in the search bar, find widget or settings - a symbol picker is available. The ['Input: Find Unicode Symbol...'](command:lean4.input.findSymbol) command can be used from the [command menu](#vsc-command-menu) or the [command palette](#vsc-command-palette). After selecting a symbol, a second dialog allows choosing between copying the symbol to the clipboard and inserting it into the active text editor.
...
The InfoView is the main interactive component of Lean. It can be used to inspect proof goals, expected types and [diagnostics](#vsc-errors-warnings-and-information), as well as render arbitrary user interfaces called ['widgets'](#vsc-widgets) for Lean code.
...
1. **Messages**. If the text cursor is positioned on a line of the span of a [diagnostic](#vsc-errors-warnings-and-information), an interactive variant of the diagnostic is displayed. Disabling the 'Lean 4 > Infoview: All Errors On Line' option will only display errors that are to the right of the text cursor.
1. **All Messages**. Always displayed. Contains interactive variants of all [diagnostics](#vsc-errors-warnings-and-information) present in the file, sorted by their proximity to the text cursor by default. The section header displays a tally of errors, warnings and informational messages separately. Can be paused by clicking the 'Pause 'All Messages'' button in the top right of the section. The sort order can be changed by clicking the 'Sort by message location' button, also in the top right of the section. The default sort order can be changed by setting the 'Lean 4 > Infoview: Message Order' option.
...
1. **Errors, warnings and information**. When hovering over a piece of code that is [underlined with a squiggly line](#vsc-errors-warnings-and-information), VS Code will display the error, warning or information associated with the squiggly line.
1. **Unicode symbols**. When hovering over a unicode symbol, VS Code will provide all available [abbreviation identifiers](#vsc-unicode-input) to input the symbol.
...
Hovers can also be triggered at the current text cursor position using the keyboard with the `Ctrl+K Ctrl+I` (`Cmd+K Cmd+I`) [chord](#vsc-chords) or the ['Show or Focus Hover'](command:editor.action.showHover) command.
...
Whether these different kinds of syntax and semantic highlighting are actually distinguishable in the editor depends on the color theme that is selected in the menu at 'File' > 'Preferences' > 'Theme' > 'Color Theme', which can also be opened by using the `Ctrl+K Ctrl+T` (`Cmd+K Cmd+T`) [chord](#vsc-chords) or the ['Preferences: Color Theme'](command:workbench.action.selectTheme) command.
...
When hovering over the left column of a Lean document editor that contains the line numbers, VS Code will display a caret next to every namespace, section and declaration. Clicking this caret, using the `Ctrl+K Ctrl+L` (`Cmd+K Cmd+L`) [chord](#vsc-chords) or the ['Toggle Fold'](command:editor.toggleFold) command will collapse this part of the document. Clicking the caret again or using the same command as before will unfold this block of code again.

Similar to the [document outline](#vsc-document-outline) or the [breadcrumbs bar](#vsc-breadcrumbs), collapsible code blocks can be used to gain an overview over a Lean document and quickly navigate it, e.g. by using the ['Fold All'](command:editor.foldAll) command or the `Ctrl+K Ctrl+0` (`Cmd+K Cmd+0`) [chord](#vsc-chords) to collapse all code blocks and later using the ['Unfold All'](command:editor.unfoldAll) command or the `Ctrl+K Ctrl+J` chord to unfold all code blocks again.
...
| :--: | 
| *Context menu when right-clicking on an identifier. Contains 'Go to Definition', 'Go to Declaration', 'Go to Type Definition', ['Find All References'](#vsc-find-references) and ['Show Call Hierarchy'](#vsc-call-hierarchy) entries.* |
...
The ['References: Find All References'](command:references-view.findReferences) command can be called by right clicking on an identifier and selecting 'Find All References' or by positioning the text cursor on an identifier and pressing `Alt+Shift+F12`. It is similar to the ['Show Call Hierarchy'](#vsc-call-hierarchy) command, but organizes usages by files, not declarations, displays a short snippet of the code where the identifier is used and cannot walk through usages recursively.
...
To use this feature, the Lean project folder must first be opened in VS Code using the ['File: Open Folder…'](command:workbench.action.files.openFolder) command that can be accessed via 'File' > 'Open Folder…' or using the `Ctrl+K Ctrl+O` (`Cmd+K Cmd+O`) [chord](#vsc-chords).
...
There are two project creation commands that can be accessed using the [command palette](#vsc-command-palette) or by navigating to the 'New Project…' submenu in the [command menu](#vsc-command-menu):
...
There are two commands to open existing Lean projects that can be accessed using the [command palette](#vsc-command-palette) or by navigating to the 'Open Project…' submenu in the [command menu](#vsc-command-menu):
...
The Lean 4 VS Code extension supports the following commands that can be run in existing Lean projects and that can be accessed using the [command palette](#vsc-command-palette) or by navigating to the 'Project Actions…' submenu in the [command menu](#vsc-command-menu):
...
This section covers how to manage different Lean versions. All of these commands provide a frontend for Lean's version manager [Elan](https://github.com/leanprover/elan/blob/master/README.md) and can be found using the [command palette](#vsc-command-palette) or by navigating to the 'Version Management…' submenu in the [command menu](#vsc-command-menu).
...
After installing Elan or updating it from a pre-4.0.0 version, the Lean 4 VS Code extension will also prompt users about whether they want to set the ['Lean 4: Always Ask Before Installing Lean Versions' setting](#vsc-elan).
...
Executing the ['Troubleshooting: Show Setup Information'](command:lean4.troubleshooting.showSetupInformation) command from the [command palette](#vsc-command-palette) or the [command menu](#vsc-command-menu) will collect information about the user's setup in the context of the last focused Lean 4 file and display it. It includes the following information:
1. Operating system
1. CPU architecture
1. CPU model
...
When Lean produces errors while interacting with a file or when an external command is executed, these errors and all external command output are displayed in the 'Lean: Editor' output view, which can be helpful to diagnose issues. It can be opened using the ['Troubleshooting: Show Output'](command:lean4.troubleshooting.showOutput) command from the [command palette](#vsc-command-palette) or the [command menu](#vsc-command-menu).
